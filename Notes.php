...
<head>
    ...
    <link rel="stylesheet" href="../CSS/ens_note.css">
...
<main>
    ...
    <div class="select-container">
        ...
        <form method="post">
            ...
            <select class="liste" id="filiere" name="filiere">
            ...
            <select class="liste" id="matiere" name="matiere">
            ...
            <div class="submitted"><br>
                ...
                <input type="submit" name="submit" class="styled" value="Envoi">
            </div>
        </form>
    ...
    if (isset($_POST['submit']))
    {
        ...
        echo"<table border='1' class='tableau-style' id='tab'><thead> <tr><th>Massar</th> <th>Nom</th>  <th>Prénom</th>   <th>Note </th>  <th> Description</th> <th>Action</th> <!--<th></th>--> </tr></thead>";
        ...
        $resultat=mysqli_query($connexion,"SELECT e.nom,e.id_massar, e.prenom, n.valeur, n.description FROM etudiant e INNER JOIN note n ON e.id_massar = n.id_massar INNER JOIN matiere m ON n.id_matiere = m.id_matiere INNER JOIN filiere f ON m.id_filiere = f.id_filiere WHERE f.libelle = '$filiere' AND m.libelle = '$matiere';");

        if($resultat)
        {
            while($row=mysqli_fetch_object($resultat))
            {
                echo "<tr align='justify'> <td> <input type='text' name='id' value='$row->id_massar' readonly> </td> <td>$row->nom</td><td>$row->prenom</td> <td> <input type='number' name='mark' value='$row->valeur'> </td> <td>$row->description</td>

                <td><a href='edit.php?id=$row->id_massar'><i class='fas fa-edit'></i></a> &nbsp&nbsp&nbsp <a href='delete.php?id=$row->id_massar' onclick=\"return confirm('Voulez-vous vraiment supprimer cet etudiant ?');\"><i class='fas fa-trash-alt'></i></a> </td>
